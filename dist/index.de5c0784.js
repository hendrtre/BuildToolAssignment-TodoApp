...
function renderTopInputSectionInDOM() {
    ...
    textInput.addEventListener("keyup", function(event) {
        if (event.code === "Enter") submitInput(event);
    });
    // New Task Add Button
    const addNewTaskButton = document.createElement("button");
    addNewTaskButton.setAttribute("class", "add btn btn-primary font-weight-bold todo-list-add-btn");
...
function toggleComplete(key) {
    ...
    if (index.complete == false) index.complete = true;
...
function renderTask(todo) {
    ...
    if (todo.complete) completeList.append(item);
    else if (todo.complete === false) //adds newly created task to incomplete ToDo List Card
    incompleteList.append(listItem);
    renderCategories();
}
//Error Checking for empty string
function promptError(removeUncategorized) {
    if (removeUncategorized == 'Uncategorized') window.alert("You cannot erase this category. You can either update the category name with the edit icon or individually edit todos with a new Category.");
    else window.alert("A To-Do item cannot be blank and a category must be selected. Please try again.");
}
function findRemovedTask(id) {
    todoArray.forEach((element)=>{
        ...
        if (element.id == id) renderTask(element);
    });
}
...
function editTodo(id) {
    const item1 = todoArray.find((item)=>item.id == Number(id)
    );
    const name = JSON.stringify(item1.name);
...
const renderCategories = ()=>{
    ...
    if (categories.length == 1 && categories[0] == 'Uncategorized') categories = [];
...
const removeCategory = (event)=>{
    ...
    if (removedCatName === 'Uncategorized') promptError(removedCatName);
    else {
        let categories = updateCategories();
...
const addCategories = (event1)=>{
    console.log(event1.target.parentElement);
    let addCategoryHTML = event1.target.parentElement;
    ...
    const getValue = (event)=>{
        /* console.log(event.target.parentElement.childNode) */ let nciValue = document.getElementById('new-cat-input').value;
    ...
    let removeIcon = document.createElement("i");
    removeIcon.setAttribute("class", ".remove mdi mdi-close-circle-outline modal-remove-icon");
    removeIcon.addEventListener("click", removeCategory);
